...
export default function MyPayslipPage() {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:5001/payroll-tracking/my-payslip", {
      credentials: "include",
    })
      .then((res) => {
        if (!res.ok) {
      ...
      .then((data) => {
        if (data) {
          ...
          const mergedPayslip = {
            ...
            deductionsDetails: { ...defaultDeductions, ...data.deductionsDetails },
          };
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:5001/payroll-tracking/download-payslip",
        {
          method: "GET",
          credentials: "include",
        }
  ...
  const handleViewTaxDetails = async () => {
    ...
    try {
      ...
      const url = `http://localhost:5001/payroll-tracking/tax-deduction/${payslipId}`;
      ...
      router.push(`/payroll-tracking/payslip-tax-details?payslipId=${payslipId}`);
    } catch (err) {
  ...
  const handleViewInsuranceDetails = () => {
    ...
    router.push(`/payroll-tracking/payslip-insurance-details?payslipId=${payslipId}`);
  };
  ...
  const handleViewMisconductDetails = () => {
    if (!payslip) return;
    router.push(`/payroll-tracking/payslip-misconduct-details?payslipId=${payslip._id}`);
  };

  // --- NEW HANDLER FOR UNPAID LEAVE DETAILS ---
  const handleViewUnpaidLeaveDetails = () => {
    if (!payslip) return;
    router.push(`/payroll-tracking/payslip-unpaid-leave-details?payslipId=${payslip._id}`);
  };
  ...
  const totalEmployeeInsuranceDeduction = (deductionsDetails.insurances ?? []).reduce(
    (s, i) => s + (grossSalaryBase * (i?.employeeRate ?? 0)) / 100,
    0
  );
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                Period:{" "}
                {payslip.createdAt ? new Date(payslip.createdAt).toLocaleDateString("en-US", {
                  year: "numeric",
                  month: "long",
                }) : "N/A"}{" "}
                  ...
                  <div className="p-3 border border-gray-200 rounded-lg">
                    {deductionsDetails.insurances.map((i) => (
                      <DetailRow
                        ...
                        value={-((grossSalaryBase * (i?.employeeRate ?? 0)) / 100)}
                        isDeduction={true}
