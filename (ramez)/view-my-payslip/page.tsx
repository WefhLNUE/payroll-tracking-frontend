...
interface Allowance { _id: string; name: string; amount: number; status: string; }
interface Tax { _id: string; name: string; rate: number; description: string; status: string; }
interface Insurance { _id: string; name: string; status: string; minSalary: number; maxSalary: number; employeeRate: number; employerRate: number; }
interface Refund { _id: string; name: string; amount: number; status: string; positionName?: string; } 
interface EarningsDetails { baseSalary: number; allowances: Allowance[]; bonuses: Refund[]; benefits: Refund[]; refunds: Refund[]; _id: string; }
interface DeductionsDetails { taxes: Tax[]; insurances: Insurance[]; penalties: any; _id: string; }
interface Payslip { 
    _id: string; 
    payrollRunId: string; 
...
const DetailRow = ({ 
    ...
    const getStatusClasses = (status?: string) => {
        ...
        if (lowerStatus === 'approved' || lowerStatus === 'active' || lowerStatus === 'paid') return 'bg-green-100 text-green-700';
        if (lowerStatus === 'pending' || lowerStatus === 'draft') return 'bg-yellow-100 text-yellow-700';
        if (lowerStatus === 'rejected' || lowerStatus === 'inactive' || lowerStatus === 'failed') return 'bg-red-100 text-red-700';
        return 'bg-gray-200 text-gray-700';
    };

    return (
        <div className={`flex justify-between items-center py-2 ${isTotal ? 'font-bold text-lg text-gray-900 border-t border-b border-gray-300 mt-2' : 'text-sm text-gray-700'}`}>
            <span className="flex items-center">
                <div className={`w-2 h-2 rounded-full mr-3 ${isDeduction ? 'bg-red-500' : 'bg-green-500'}`}></div>
                {label}
                {/* Display Status Label */}
                {statusLabel && (
                    <span 
                        className={`ml-2 px-2 py-0.5 text-xs font-semibold rounded-full uppercase ${getStatusClasses(statusLabel)}`}
                    >
                        ({statusLabel})
                    </span>
                )}
...
export default function MyPayslipPage() {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:3000/payroll-tracking/my-payslip", { credentials: "include" })
      .then(res => {
        if (!res.ok) {
          if (res.status === 404) {
            return null;
  ...
  const handleDownload = async () => {
    ...
    try {
        const response = await fetch("http://localhost:3000/payroll-tracking/download-payslip", {
            method: 'GET',
            credentials: 'include',
        });
  ...
  const handleViewTaxDetails = async () => {
    ...
    try {
        ...
        const url = `http://localhost:3000/payroll-tracking/tax-deduction/${payslipId}`;
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({ message: 'Unknown error fetching tax details.' }));
            throw new Error(errorData.message || `Failed to fetch tax details (Status: ${response.status})`);
        }
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-gray-100"><p className="text-center text-xl text-blue-600 font-semibold animate-pulse">Loading payroll data...</p></div>;
  
  if (error && error !== 'Failed to fetch payslip with status: 404' && error !== 'Failed to fetch payslip (Status: 404)') return <div className="min-h-screen flex items-center justify-center bg-gray-100"><p className="text-center text-xl text-red-600 font-bold p-8 bg-white rounded-lg border-t-4 border-red-500 shadow-lg">Error: {error}</p></div>;
  ...
  if (!payslip) return (
    <div className="min-h-screen flex items-center justify-center p-8 bg-gray-100">
      <div className="text-center p-10 bg-white rounded-xl shadow-2xl border-t-4 border-yellow-500">
        ...
        <p className="text-2xl font-bold text-gray-800 mb-2">No Recent Payslip Found</p>
        <p className="text-gray-600">The most recent payroll statement is not yet available in the system.</p>
        <button 
  ...
  const totalBonuses = earningsDetails.bonuses.reduce((s, b) => s + b.amount, 0);
  const totalRefunds = earningsDetails.refunds.reduce((s, r) => s + r.amount, 0);
  const totalBenefits = earningsDetails.benefits.reduce((s, b) => s + b.amount, 0);
  ...
  const totalTaxes = deductionsDetails.taxes.reduce((s, t) => s + (grossSalaryBase * t.rate / 100), 0);
  const totalEmployeeInsuranceDeduction = deductionsDetails.insurances.reduce((s, i) => s + (grossSalaryBase * i.employeeRate / 100), 0);
  const totalPenalties = deductionsDetails.penalties?.penalties?.reduce((s: number, p: any) => s + p.amount, 0) || 0;
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center py-5 px-8 bg-white rounded-xl shadow-lg border-l-4 border-blue-500">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Payroll Statement</h1>
              <p className="text-sm text-gray-500 mt-0.5">Period: {new Date(payslip.createdAt).toLocaleDateString("en-US", { year: 'numeric', month: 'long' })} | ID: {payslip.payrollRunId}</p>
            </div>
          </div>
          <div className={`mt-4 sm:mt-0 px-4 py-1.5 text-sm font-bold rounded-full ${statusClasses}`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
          ...
          <div className="bg-white rounded-xl p-5 shadow-lg border-t-4 border-blue-500 transition hover:shadow-xl relative">
            ...
            <p className="mt-1 text-2xl font-bold text-gray-900">
              ${earningsDetails.baseSalary.toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl p-5 shadow-lg border-t-4 border-red-500 transition hover:shadow-xl">
            ...
            <p className="mt-1 text-2xl font-bold text-gray-900">
              ${payslip.totaDeductions.toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </p>
          </div>
          ...
          <div className="bg-cyan-500 text-white 
            ...
            <p className="mt-1 text-2xl font-extrabold">
              ${payslip.totalGrossSalary.toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </p>
          </div>

          {/* 5. Unused Leave Compensation Card (NEW) */}
          <div className="bg-white rounded-xl p-5 shadow-lg border-t-4 border-emerald-500 transition hover:shadow-xl relative group cursor-pointer" onClick={handleViewLeaveCompensation}>
            <div className="flex items-center mb-3">
            ...
            <button
                onClick={(e) => { e.stopPropagation(); handleViewLeaveCompensation(); }}
            ...
            <p className="mt-1 text-2xl font-bold text-gray-900">Potential Value</p>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6 pt-8">
          ...
          <section className="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div className="p-6 space-y-4">
                <DetailRow label="Base Salary" value={earningsDetails.baseSalary} isTotal={true} />

                {/* Allowances Section - Display all, filter in total calculation */}
                {earningsDetails.allowances.length > 0 && (
                  <div className="pt-2">
                    <h3 className="font-semibold text-lg text-gray-800 mb-1">Allowances</h3>
                ...
                {(totalBonuses > 0 || totalRefunds > 0 || totalBenefits > 0) && (
                  <div className="pt-2">
                    <h3 className="font-semibold text-lg text-gray-800 mb-1">Other Income</h3>
                    <div className="p-3 border border-gray-200 rounded-lg">
                      {earningsDetails.bonuses.map(b => (
                        <DetailRow key={b._id} label={`Bonus: ${b.positionName || b.name}`} value={b.amount} statusLabel={b.status} />
                      ))}
                      {earningsDetails.refunds.map(r => (
                        <DetailRow key={r._id} label={`Refund: ${r.name}`} value={r.amount} statusLabel={r.status} />
                      ))}
                      {earningsDetails.benefits.map(b => (
                        <DetailRow key={b._id} label={`Benefit: ${b.name}`} value={b.amount} statusLabel={b.status} />
                      ))}
                    </div>
                  </div>
                )}
          ...
          <section className="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div className="p-6 space-y-4">
                    ...
                    <div className="flex justify-between items-center mb-1">
                        ...
                        <button 
                            ...
                            className={`flex items-center text-sm font-medium p-1 rounded transition 
                                ...
                                ${isFetchingTaxDetails ? 'opacity-50 cursor-not-allowed' : ''}`}
                        >
                            {isFetchingTaxDetails ? 'Loading...' : 'View Tax Details'}
                            {!isFetchingTaxDetails && <ChevronRight className="w-4 h-4 ml-1" />}
                        </button>
                    </div>
                ...
                <div className="pt-2 pb-2">
                  <div className="flex justify-between items-center mb-1">
                    <h3 className="font-semibold text-lg text-gray-800">Penalties</h3>
                  ...
                  {totalPenalties > 0 && deductionsDetails.penalties?.penalties?.length > 0 ? (
                    <div className="p-3 border border-gray-200 rounded-lg">
                      {deductionsDetails.penalties.penalties.map((p: any, index: number) => (
                        <DetailRow 
                            key={index} 
                            label={p.reason} 
                            value={-p.amount} 
        ...
        <div className="flex justify-end gap-4 mt-8 pt-4 border-t border-gray-300">
          <button 
            ...
            className={`flex items-center px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg shadow-md transition ${isDownloading ? 'opacity-60 cursor-not-allowed' : 'hover:bg-blue-700'}`}
          >
            {isDownloading ? (
                <>
