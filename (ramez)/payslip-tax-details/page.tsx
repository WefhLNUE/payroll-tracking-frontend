...
import { ChevronLeft, DollarSign, Activity, FileText, Scale } from 'lucide-react';
...
export default function PayslipTaxDetailsPage() {
    ...
    const [taxDetails, setTaxDetails] = useState<DetailedTaxDeduction | null>(null);
    ...
    useEffect(() => {
        ...
        const fetchDetails = async () => {
            try {
                ...
                if (!response.ok) {
                    const errorBody = await response.json().catch(() => ({ message: 'Server error occurred.' }));
                    throw new Error(errorBody.message || `Failed to load tax details (Status: ${response.status})`);
                }
    ...
    if (loading) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center p-8 bg-gray-100">
                ...
                <p className="text-xl font-semibold text-blue-600">Loading detailed tax breakdown...</p>
            </div>
        );
    }
    ...
    if (!payslipId || !taxDetails) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center p-8 bg-gray-100">
                <p className="text-xl text-red-600 font-bold mb-4">Tax details not found or failed to load.</p>
                <button 
                    onClick={() => router.back()}
                ...
                <div className="space-y-8">
                    ...
                    <div className="p-5 bg-green-50 rounded-lg border-l-4 border-green-500 shadow-sm">
                        ...
                        <p className="text-4xl font-extrabold text-green-700">
                            ${(taxDetails.taxableBase ?? 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                        </p>
                        <p className="text-sm text-gray-600 mt-2">This is the gross income amount used to calculate percentage-based tax deductions.</p>
                    </div>
                    ...
                    <div className="p-5 bg-red-50 rounded-lg border-l-4 border-red-500 shadow-sm">
                        ...
                        <p className="text-4xl font-extrabold text-red-700">
                            ${(taxDetails.totalTax ?? 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                        </p>
                        <p className="text-sm text-gray-600 mt-2">Sum of all mandatory tax and social contributions.</p>
                    </div>
                    
                    <h2 className="text-2xl font-semibold text-gray-800 border-b pb-2">Individual Tax Breakdown</h2>
                    <ul className="space-y-4">
                        {taxDetails.taxes?.map((tax, index) => (
                            ...
                            >
                                <div>
                                    <span className="font-bold text-lg text-gray-800">{tax.name} ({tax.rate}%)</span>
                                ...
                                <span className="font-extrabold text-xl text-red-700 mt-2 sm:mt-0">
                                    ...
                                    - ${ (tax.amount ?? 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                                </span>
                            </li>
                        ))}
                    </ul>
