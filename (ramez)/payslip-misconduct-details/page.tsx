...
function MisconductContent() {
    ...
    const [viewingRange, setViewingRange] = useState({ start: "", end: "", label: "" });

    const fetchDeductions = useCallback(async (startDate: Date, endDate: Date) => {
        setLoading(true);
        setError("");
        ...
        try {
            ...
            const url = `http://localhost:3000/payroll-tracking/misconduct-deductions?startDate=${startISO}&endDate=${endISO}`;
    ...
    useEffect(() => {
        const init = async () => {
            try {
                const payslipRes = await fetch(`http://localhost:3000/payroll-tracking/my-payslip`, { credentials: "include" });
                ...
                const start = new Date(pDate.getFullYear(), pDate.getMonth(), 1, 0, 0, 0);
                const end = new Date(pDate.getFullYear(), pDate.getMonth() + 1, 0, 23, 59, 59);
    ...
    if (loading) return (
        <div className="min-h-screen flex items-center justify-center bg-gray-50">
            <div className="text-center">
                ...
                <p className="text-gray-500 font-medium">Querying Attendance Records...</p>
            </div>
        </div>
    );
                ...
                <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-200 mb-8">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div className="flex items-center gap-5">
                            ...
                            <div>
                                <h1 className="text-3xl font-bold tracking-tight">Deduction Details</h1>
                                <p className="text-slate-500">Attendance misconduct & policy violations</p>
                            </div>
                        </div>
                        <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 min-w-[180px]">
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Viewing Period</p>
                ...
                {error ? (
                    ...
                    <div className="bg-white rounded-3xl p-20 text-center border-2 border-dashed border-slate-200">
                        ...
                        <p className="text-slate-500 mb-8 max-w-sm mx-auto leading-relaxed">
                            We couldn't find any misconduct records for <span className="font-bold">{viewingRange.label}</span>. 
                            If you have recent records, they might be in the current month.
                        </p>
                        <button 
                    ...
                    <div className="space-y-4">
                        {deductions.map((item, index) => (
                            <div key={index} className="group bg-white rounded-2xl border border-slate-200 hover:border-red-200 transition-all overflow-hidden shadow-sm hover:shadow-md">
                                <div className="flex">
                                    <div className={`w-2 ${item.type === 'Absenteeism' ? 'bg-red-500' : 'bg-amber-400'}`} />
                                    <div className="p-6 w-full flex flex-col md:flex-row justify-between gap-4">
                                        <div className="space-y-3">
                                            <div className="flex items-center gap-3">
                                                <span className={`text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-widest ${
                                                    item.type === 'Absenteeism' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'
                                                }`}>
                                                    {item.type}
                                                </span>
                                        ...
                                        {item.potentialDeductionAmount && (
                                            <div className="bg-red-50/50 px-6 py-4 rounded-2xl border border-red-100 flex flex-col justify-center items-end">
                                                <span className="text-[10px] font-bold text-red-400 uppercase tracking-tighter">Amount</span>
                                                <span className="text-2xl font-black text-red-600">
                                                    -${item.potentialDeductionAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                                                </span>
                                            </div>
                                        )}
                                    </div>
                        ...
                        <div className="bg-slate-900 rounded-3xl p-8 text-white flex flex-col md:flex-row justify-between items-center gap-6 mt-12 shadow-2xl">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Aggregate Deduction</p>
                                    <p className="text-slate-300 text-sm">Calculated impact for {viewingRange.label}</p>
                                </div>
                            </div>
                            <div className="text-4xl font-black text-red-400">
                                -${deductions.reduce((acc, curr) => acc + (curr.potentialDeductionAmount || 0), 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                            </div>
                        </div>
                    </div>
                )}
...
export default function MisconductPage() {
    return (
        <Suspense fallback={<div className="p-20 text-center text-slate-400 animate-pulse">Loading Application Context...</div>}>
            <MisconductContent />
        </Suspense>
    );
}
