...
// Lucide icons
import { ArrowLeft, FileText, Loader2, DollarSign, Clock, Calendar, TrendingDown, BarChart2, CheckCircle, XCircle, TrendingUp } from 'lucide-react'; 
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-[30%_1fr_20%] divide-y md:divide-y-0 md:divide-x divide-gray-200">
                ...
                <div className="p-4 flex flex-col justify-center bg-gray-100"> 
                    ...
                    <div className="flex justify-between items-center mb-1">
                        <div className="flex items-center space-x-2">
                            ...
                            <span className="text-lg font-extrabold text-gray-900">{monthName} {item.year}</span>
                        </div>
                ...
                <div className="py-4 px-6 flex justify-between items-center text-center bg-white">
                    <div className="flex flex-col flex-1 border-r border-gray-100 pr-4">
                        ...
                        <span className="text-xl font-extrabold text-gray-900">{item.totalGrossSalary}</span>
                    </div>
                    <div className="flex flex-col flex-1 pl-4">
                        <span className="text-xs font-semibold text-gray-500 flex items-center mb-1 justify-center">
                            <TrendingDown className="w-3 h-3 mr-1 text-red-600" /> DEDUCTIONS
                        </span>
                        <span className="text-xl font-extrabold text-red-700">{item.totalDeductions}</span>
                    </div>
                </div>
...
export default function PayslipHistoryPage() {
    ...
    useEffect(() => {
        ...
        fetch("http://localhost:3000/payroll-tracking/my-payslip-status", { credentials: "include" })
            .then(res => {
                if (res.status === 404) {
                    return [];
                }
                if (!res.ok) {
                    throw new Error("Failed to fetch payslip history (Status: " + res.status + ")");
                }
    ...
    if (loading) return (
        <div className="min-h-screen flex items-center justify-center bg-gray-100">
            ...
            <p className="text-xl text-slate-600 font-semibold">Loading payment history...</p>
        </div>
    );
    
    if (error) return (
        <div className="min-h-screen flex items-center justify-center bg-gray-100 p-8">
            <p className="text-center text-xl text-red-600 font-bold p-8 bg-white rounded-lg border-t-4 border-red-500 shadow-lg">Error loading data: {error}</p>
        </div>
    );
            ...
            <div className="space-y-8">
                ...
                {history.length === 0 ? (
                    ...
                    <div className="max-w-7xl mx-auto p-20 text-center bg-white rounded-xl shadow-lg border-t-4 border-amber-500">
                        ...
                        <p className="text-2xl font-semibold text-gray-700">No Past Records Available</p>
                        <p className="text-gray-500 mt-2">The system has not generated any previous payroll statements for your account.</p>
                    </div>
                ) : (
                    <div className="p-0">
