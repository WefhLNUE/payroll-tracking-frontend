...
function UnpaidLeaveContent() {
    ...
    const fetchDetails = async () => {
        ...
        setError("");
        try {
            const res = await fetch(`http://localhost:3000/payroll-tracking/unpaid-leave-deductions`, { 
                credentials: "include" 
            });

            if (!res.ok) {
                throw new Error(`Server error: ${res.status} - Could not retrieve leave data.`);
            }
    ...
    if (loading) return (
        <div className="min-h-screen flex items-center justify-center bg-gray-50">
            <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-gray-500 font-medium italic">Calculating unpaid leave impact...</p>
            </div>
        </div>
    );
            ...
            <div className="max-w-4xl mx-auto">
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-8 mb-6">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div className="flex items-center gap-5">
                            ...
                            <div>
                                <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Unpaid Leave Details</h1>
                                <p className="text-slate-500 mt-1 font-medium">Breakdown of salary deductions for non-paid absences</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {error ? (
                    <div className="bg-red-50 border-l-4 border-red-500 p-6 rounded-r-2xl flex items-center shadow-sm">
                        ...
                        <button onClick={fetchDetails} className="ml-auto bg-red-600 text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-red-700 transition">
                            Retry
                        </button>
                    </div>
                    ...
                    <div className="bg-white rounded-3xl p-20 text-center border-2 border-dashed border-slate-200">
                        ...
                        <h3 className="text-xl font-bold text-slate-900 mb-2">No Unpaid Absences</h3>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Base Salary</span>
                                </div>
                                <p className="text-xl font-bold text-slate-900">${data.baseSalary.toLocaleString()}</p>
                            </div>
                            
                            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                                <div className="flex items-center gap-3 mb-2">
                                    <CalendarDays className="w-4 h-4 text-purple-500" />
                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Working Days</span>
                                </div>
                                <p className="text-xl font-bold text-slate-900">{data.workingDays} Days</p>
                            </div>
                            ...
                            <div className="bg-blue-600 p-6 rounded-2xl shadow-lg border border-blue-700">
                                <div className="flex items-center gap-3 mb-2 text-blue-100">
                                    ...
                                    <span className="text-[10px] font-bold uppercase tracking-widest">Daily Rate</span>
                                </div>
                                <p className="text-xl font-bold text-white">${data.dailyRate.toLocaleString()}</p>
                            </div>
                        </div>
                        ...
                        <div className="grid gap-4">
                            {data.details.map((item, index) => (
                                <div key={index} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-all">
                                    <div className="flex">
                                        ...
                                        <div className="p-6 w-full flex flex-col md:flex-row justify-between items-center gap-6">
                                            <div className="space-y-2 flex-1">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <h3 className="text-lg font-bold text-slate-800">{item.leaveType}</h3>
                                                </div>
                                            ...
                                            <div className="text-right min-w-[140px] bg-slate-50 px-6 py-4 rounded-2xl border border-slate-100">
                                                <p className="text-[10px] font-bold text-slate-400 uppercase mb-1 tracking-tighter">Line Total</p>
                        ...
                        <div className="bg-slate-900 rounded-[2rem] p-8 text-white shadow-2xl flex flex-col md:flex-row justify-between items-center gap-8 relative overflow-hidden">
                            ...
                            <div className="flex items-center gap-6 relative z-10">
                                ...
                                <div>
                                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Aggregate Impact</p>
                                    <p className="text-slate-300 text-sm mt-1">Total of {data.unpaidDaysTotal} unpaid days deducted</p>
                                </div>
                            </div>
                        ...
                        <div className="p-6 bg-slate-100/50 rounded-2xl border border-slate-200 flex gap-4 items-start">
                            ...
                            <p className="text-xs text-slate-500 leading-relaxed font-medium">
                                <strong>Calculation Note:</strong> Your daily rate is calculated as: 
                                <code className="mx-1 bg-white px-1.5 py-0.5 rounded border border-slate-200 text-blue-600 font-bold">
                                    Base Salary (${data.baseSalary.toLocaleString()}) / {data.workingDays} Working Days
                                </code>. 
                                This rate (${data.dailyRate}) is applied to each day of unpaid leave recorded in the system.
                            </p>
                        </div>
                    </div>
                )}
    ...
    return (
        <Suspense fallback={
            <div className="min-h-screen flex items-center justify-center">
                <p className="text-slate-400 animate-pulse font-medium tracking-widest uppercase text-xs">Loading Secure Data...</p>
            </div>
        }>
            <UnpaidLeaveContent />
        </Suspense>
