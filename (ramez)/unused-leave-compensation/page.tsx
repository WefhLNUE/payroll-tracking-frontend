...
// --- API FETCH FUNCTION ---
const API_URL = "http://localhost:3000/payroll-tracking/unused-leave-compensation";
    ...
    if (loading) return (
        <div className="min-h-screen flex items-center justify-center bg-gray-100">
            <div className="text-center">
                ...
                <p className="mt-4 text-blue-600 font-semibold">Calculating leave values...</p>
            </div>
        </div>
    );
            ...
            <div className="max-w-5xl mx-auto space-y-6">
                ...
                <header>
                    ...
                    <div className="flex items-center bg-white p-6 rounded-xl shadow-md border-l-4 border-emerald-500">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-gray-900">Unused Leave Compensation</h1>
                            <p className="text-sm text-gray-500">Estimating the cash value of your remaining leave balance</p>
                        </div>
                    </div>
                </header>
                ...
                <div className="bg-gradient-to-br from-emerald-600 to-teal-700 text-white p-8 rounded-2xl shadow-xl flex flex-col sm:flex-row justify-between items-center">
                    <div className="mb-4 sm:mb-0">
                        <h2 className="text-emerald-100 font-medium uppercase tracking-wider text-sm">Total Potential Encashment</h2>
                        <p className="text-4xl font-black mt-1">{formatCurrency(data.totalPotentialCompensation)}</p>
                    </div>
                    <div className="bg-white/20 p-4 rounded-lg backdrop-blur-sm text-right">
                        <p className="text-xs text-emerald-50 text-emerald-100 uppercase">Daily Pay Rate</p>
                        <p className="text-xl font-bold">{formatCurrency(data.dailyRate)} / day</p>
                    </div>
                </div>
                                ...
                                {data.leaveBreakdown.map((item, idx) => (
                                    <tr key={idx} className="hover:bg-gray-50 transition">
                                        <td className="px-6 py-4">
                                            <p className="font-semibold text-gray-800">{item.leaveTypeName}</p>
                                            <p className="text-xs text-gray-400">Accrued: {item.accruedActual} days</p>
                                        </td>
                                        <td className="px-6 py-4">
                                            {item.isEncashable ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-white p-6 rounded-xl shadow border border-gray-100">
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between text-sm">
                                ...
                                <span className="font-semibold">{formatCurrency(data.monthlyBaseSalary)}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                            ...
                            <div className="border-t pt-2 flex justify-between text-sm">
                                <span className="text-gray-800 font-bold">Calculated Daily Rate</span>
                                <span className="text-blue-600 font-bold">{formatCurrency(data.dailyRate)}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-amber-50 p-6 rounded-xl border border-amber-100">
                        ...
                        <p className="text-xs text-amber-700 leading-relaxed">
                            {data.note} 
                            <br /><br />
                            Please note that "Total Potential Compensation" is an estimate. Actual encashment 
                            is subject to company policy, tax deductions, and HR approval during the final payroll run.
                        </p>
                    </div>
                </div>
