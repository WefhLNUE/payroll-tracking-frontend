...
import { DollarSign, FileText, Briefcase, BarChart2, Clock, ChevronLeft } from 'lucide-react'; 
...
const fetchBaseSalaryData = async (): Promise<BaseSalaryAPIResponse> => {
    try {
        ...
        throw new Error("Could not connect to the payroll service or authentication failed.");
    }
};
...
const DetailItem: React.FC<DetailItemProps> = ({ icon: Icon, label, value, highlight = false }) => (
    <div className={`flex items-start space-x-4 p-4 border-b last:border-b-0 ${highlight ? 'bg-blue-50 border-blue-200' : 'border-gray-200'}`}>
        <Icon className={`w-6 h-6 ${highlight ? 'text-blue-600' : 'text-blue-500'} mt-0.5 flex-shrink-0`} />
        <div>
            <p className="text-sm font-medium text-gray-500">{label}</p>
            <p className={`text-base font-semibold text-gray-900 mt-0.5 ${highlight ? 'font-bold' : ''}`}>{value}</p>
        </div>
    </div>
);
...
export default function ContractDetailsPage() {
    ...
    // --- No Pay Grade Assigned ---
if (!details.hasPayGrade) {
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="bg-white p-8 rounded-xl shadow-lg text-center">
        <p className="text-xl font-semibold text-gray-700">
          Your base salary is not configured yet
        </p>
        <p className="mt-2 text-gray-500">
          Please contact HR or Payroll administration.
        </p>
      </div>
    </div>
  );
    ...
    const formatString = (s: string) => 
        s.toLowerCase().replace(/_/g, ' ').replace(/-/g, ' ').split(' ')
            ...
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                <header className="py-5">
                    ...
                    <div className="mt-4 flex items-center bg-white p-6 rounded-xl shadow-lg border-l-4 border-blue-600">
                        ...
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900">Base Compensation Details</h1>
                            <p className="text-sm text-gray-500 mt-0.5">Data retrieved for Employee ID: **{details.employeeId}**</p>
                        </div>
                    </div>
                </header>
                ...
                <div className="mt-8 p-6 bg-yellow-50 border-l-4 border-yellow-500 rounded-lg shadow-sm text-sm text-gray-700">
                    ...
                    <p className="mt-2">This page displays all data returned directly from the authenticated base salary API endpoint (`/payroll-tracking/base-salary`). No other employment details are currently available through this specific endpoint.</p>
                </div>
