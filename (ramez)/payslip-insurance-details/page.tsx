...
import { Shield, DollarSign, Loader2, AlertTriangle, ArrowLeft, TrendingUp, TrendingDown, Layers, Percent, Zap } from 'lucide-react'; 
...
const SummaryCard = ({ title, value, icon: Icon, colorHex, textClass, delay }: { title: string; value: number; icon: React.ElementType; colorHex: string; textClass: string; delay: number }) => {
    ...
    return (
        <div 
            className={`bg-white p-6 rounded-xl shadow-lg border-t-4 transition-all duration-700 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`} 
            style={{ borderColor: colorHex }}
        >
            <div className="flex items-center mb-3">
                <div className="p-3 rounded-full transition-colors duration-300" style={{ backgroundColor: `${colorHex}15` }}> 
                    <Icon className={`w-6 h-6 ${textClass}`} style={{ color: colorHex }} />
                </div>
            </div>
            <p className="text-sm font-medium text-gray-500">{title}</p>
            <p className={`mt-1 text-3xl font-extrabold ${textClass}`} style={{ color: colorHex }}>
...
const InsuranceRow = ({ detail, index }: { detail: InsuranceDetail, index: number }) => (
    <div 
        ...
        style={{ animation: `fadeIn 0.5s ease-out ${0.2 + index * 0.05}s forwards` }} // Staggered row animation
    >
    ...
    useEffect(() => {
        ...
        const fetchInsuranceDetails = async () => {
            ...
            try {
                ...
                const response = await fetch("http://localhost:3000/payroll-tracking/insurance-deductions", {
                    method: 'GET',
                    credentials: 'include', 
                ...
                if (!response.ok) {
                    ...
                    throw new Error(errorData.message || `Failed to fetch insurance details (Status: ${response.status})`);
                }
    ...
    if (loading) {
        return (
            <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: Colors.Background }}>
                ...
                <p className="text-xl text-indigo-600 font-semibold">Retrieving insurance details...</p>
            </div>
        );
    }
    ...
    if (error || !data) {
        return (
            <div className="min-h-screen flex items-center justify-center p-8" style={{ backgroundColor: Colors.Background }}>
                <div className="text-center p-10 bg-white rounded-xl shadow-2xl border-t-4 border-red-600" style={{ animation: 'fadeIn 0.5s ease-out forwards' }}>
                    <AlertTriangle className="w-16 h-16 mx-auto text-red-600 mb-4" />
                    <p className="text-2xl font-bold text-gray-800 mb-2">Data Fetch Error</p>
        ...
        <div className="min-h-screen p-4 sm:p-8" style={{ backgroundColor: Colors.Background }}>
            <div className="max-w-6xl mx-auto space-y-8">
                ...
                <header 
                    ...
                    style={{ borderColor: Colors.PrimaryBlue, animation: 'fadeIn 0.5s ease-out 0s forwards' }}
                >
                    <button 
                        onClick={() => window.history.back()} 
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                        Payslip ID: **{payslipId || data.payslipId}** | Payroll Run: **{data.payrollRunId}**
                    </p>
                </header>
                ...
                >
                    <div className="p-6">
                        <h2 className="text-xl font-bold text-gray-800 mb-4 pb-3 border-b flex items-center">
                            ...
                            Policy Breakdown (Based on {data.grossSalary ? 'Gross Salary' : 'Base Salary'}: ${formatCurrency(data.grossSalary || data.baseSalary)})
                        </h2>
                        ...
                        <div className="grid grid-cols-12 gap-4 py-3 font-bold text-xs uppercase text-gray-700 bg-indigo-50/70 rounded-lg shadow-inner">
                            ...
                            <div className="col-span-3 text-right pr-4">Employer Contribution</div>
                        </div>

                        {/* Rows - Staggered Fade In applied via InsuranceRow component */}
                        <div className="divide-y divide-gray-200">
                            {data.insurances.map((insurance, index) => (
                                <InsuranceRow key={insurance.name} detail={insurance} index={index} />
                            ))}
                        </div>
                    </div>
                </section>
                ...
                >
                    ...
                    <p>
                        Your contribution amount (**Deduction**) is subtracted from your gross pay. The Employer's Contribution is an additional cost borne by the company, not included in your gross earnings.
                    </p>
                </div>
            </div>
        </div>
